...
densify::densify(Map &imap, ATANCamera &iCam, float ilembda, double ipatch_size, int ipatch_resolution, int iimSize1, int iimSize2, int ino_of_frames):dmap(imap),Cam(iCam){

	cc_lembda = ilembda;               // image match threshold
	patch_size = ipatch_size;             // size of 3d patch
...
double distance(cv::Point2f A, cv::Point2f B){
	double mag = sqrt(pow(A.x - B.x,2) + pow(A.y - B.y,2) );
	return mag;
}
...
//	Vector<3> A = cross_product(camfwrld*patch[1] - camfwrld*patch[0], camfwrld*patch[3] - camfwrld*patch[0]);
//	Vector<3> B;
//	B[0] = 0; B[1] = 0; B[2] = 1;
//	float ang = A*B;
//	ang = ang/magnitude(B);
...
//	if(patchInIm && (distance(impatch[0], impatch[1]) < 2.0) && (distance(impatch[0], impatch[2]) < 2.0) )
//		patchInIm = 0;
	return impatch;
}
...
void densify::add_3Dpoints(vector<Vector<3> > patch, cv::Mat image){
	...
	U1 = U1/magnitude(U1);
	V1 = V1/magnitude(V1);
	pcl::PointCloud<pcl::PointXYZRGB> colcloud;
	for(int i =0; i < patch_resolution; i=i+1)
		for(int j = 0; j < patch_resolution; j=j+1){
			P = (U1*i*patch_size + V1*j*patch_size)/patch_resolution + patch[0];     // need to be corrected ---- logically 
			pcl::PointXYZRGB PI;
			PI.x = P[0]; PI.y = P[1]; PI.z =P[2];
			PI.r = image.at<uchar>(j+1, i+1); PI.b = image.at<uchar>(j+1, i+1); PI.g = image.at<uchar>( j+1, i+1);
			colcloud.push_back(PI);
		}
	colcloudT +=colcloud;
}
...
pcl::PointCloud<pcl::PointXYZ>::Ptr densify::make_dense(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud, int keyframeno){
	...
	for(int l = 0; l < grid.size(); l++){
		...
		cv::Mat warpedImPatch = warp_patch(&(cv::Mat(imSize[1], imSize[0], CV_8UC1, dmap.vpKeyFrames[keyframeno]->aLevels[0].im.data())), homography);
//	cv::imshow("1", warpedImPatch);
		...
		for(int i = 1; i < no_of_frames; i++){
			...
			cv::Mat currWarpedImPatch = warp_patch(&(cv::Mat(imSize[1], imSize[0], CV_8UC1, dmap.vpKeyFrames[keyframeno-i]->aLevels[0].im.data())), homography);                   // get warp_factor
//	cv::imshow("2", currWarpedImPatch);
//	cv::waitKey(1);
			bool matchResult = cross_corelation(currWarpedImPatch, warpedImPatch);
			if (!matchResult && (onPlaneBeleif > 3))
				onPlaneBeleif--;
			else if (matchResult)
				onPlaneBeleif++;
			else
