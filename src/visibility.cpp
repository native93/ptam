...
VisibilityCalculatorServer::VisibilityCalculatorServer()
{
    ...
    ugv_map_sub = nh.subscribe("/map", 10, &VisibilityCalculatorServer::ugv_map_callback, this);
...
mat.getEulerZYX(yaw,pitch,roll);
    ...
    for(float theta = yaw-0.534; theta <= yaw+0.534; theta+= 0.08) {
        ...
        for(float r = 0; r < R_MAX; r+=0.1) {
           ...
           // pt.setZ(0.0);
           // pt.setW(1.0);
            pt_transformed.setX(p.position.x+(r*cos(theta)));

            pt_transformed.setX(p.position.y+(r*sin(theta)));
