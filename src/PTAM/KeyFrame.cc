...
void KeyFrame::MakeKeyFrame_Lite(BasicImage<CVD::Rgb<byte> > &imRGB , BasicImage<byte> &im)
{
  ...
  // copy the RGB image 
  this->imRGB.resize(imRGB.size());
  copy(imRGB, this->imRGB);
  ...
  for(int i=0; i<LEVELS; i++)
	...
	fast_corner_detect_10(lev.im, lev.vCorners, 10);
      ...
      if(i == 0)
	fast_corner_detect_10(lev.im, lev.vCorners, 1);
      if(i == 1)
	fast_corner_detect_10(lev.im, lev.vCorners, 1);
      if(i == 2)
	fast_corner_detect_10(lev.im, lev.vCorners, 1);
