...
//message_filters::Subscriber<sensor_msgs::Image>sub(it_,input_topic_name, 1);
  ...
  odom_sub.subscribe(nh,"/odom",100);
//altitude_sub.subscribe(nh,"/ardrone/navdata_altitude",100);
...
message_filters::Synchronizer<MySyncPolicy>sync(MySyncPolicy(20),sub,odom_sub);
sync.registerCallback(boost::bind(&VideoSource::odom_callback,this, _1, _2));
...
*/
void VideoSource::odom_callback(const sensor_msgs::ImageConstPtr &img_msg,const nav_msgs::Odometry::ConstPtr &msg)
{
...
//sync.connectInput(sub,altitude_sub);
//ofstream myfile;
//myfile.open("/home/irl/workspace/ptam/odometry.csv",ios::app);
...
//ss<<"/home/irl/dataset_ptam/images/"<<counter_image<<".jpg";
counter_image+=1;
//img_save(imRGB,ss.str(),CVD::ImageType::JPEG);
...
*pos_x=odom_pose.position.x;
if(*pos_x>1.5&&!fkf_select){
    fkf_select=true;
    *first_kf_ptr=true;
    pos_fkf=*pos_x;
}
if((*pos_x-pos_fkf>0.15)&&(fkf_select)&&(!skf_select)){
    skf_select=true;
    *second_kf_ptr=true;
}

//myfile<<odom_pose.position.x<<","<<odom_pose.position.y<<endl;

*pos_y=odom_pose.position.y;
return true;
}
